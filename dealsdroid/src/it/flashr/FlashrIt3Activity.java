 ...
 */
public class FlashrIt3Activity extends MyListActivity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		int tokenAvailable = (new Integer(SP.getString("token", "1"))).intValue();
		Toast.makeText(this, SP.getString("token", "1"), Toast.LENGTH_LONG).show();
		 
		
		if(tokenAvailable != 1) {
			Intent iExp = new Intent(this, ActivationActivity.class);
			startActivity(iExp);
		}*/
		
		super.onCreate(savedInstanceState);
		Toast.makeText(FlashrIt3Activity.this, FlashrUtils.getSHA256("toto"), Toast.LENGTH_SHORT).show();
		System.out.println(FlashrUtils.getSHA256("toto"));
		arrayAdapter = new EndlessListAdapter(this, R.layout.listrow,
	...
	private List<ListElement> retrieveItems(Integer itemNumber,
		...
		String jsonInput = FlashrUtils.retrieveHardCodedDealsJSON();
		try {
			JSONArray jsonArray = new JSONArray(jsonInput);
			for (int i = 0; i < jsonArray.length(); i++) {
				JSONObject jsonObject = jsonArray.getJSONObject(i);
				System.out.println("Deal_id: >>>>>>>>>>>>>>>>" + jsonObject.getString("deal_id"));
				System.out.println("Deal title: >>>>>>>>>>>>>>>>" + jsonObject.getString("title"));
				System.out.println("Deal code: >>>>>>>>>>>>>>>>" + jsonObject.getString("deal_code"));
				String itemToBeAdded = getString(R.string.deal_item,jsonObject.getString("title") + " Number " + jsonObject.getString("deal_id"));
	...
	class Downloaditems extends AsyncTask<Integer, Void, List<ListElement>> {
		...
		protected List<ListElement> doInBackground(Integer... params) {
			...
			return retrieveItems(params[ITEM_NUMBER_INDES],
